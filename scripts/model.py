...
class AcousticLeakNet(nn.Module):
    ...
    def __init__(self, signal_length: int = 2000, n_scalars: int = 9,
...
if __name__ == "__main__":
    model = AcousticLeakNet(signal_length=8000, n_scalars=9, base_channels=64)
    print(f"AcousticLeakNet parameters: {count_parameters(model):,}")

    # Forward pass test
    B = 4
    signal  = torch.randn(B, 2, 8000)
    scalars = torch.randn(B, 9)
