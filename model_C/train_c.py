...
==============================================
Reads from ../cache_c/
Saves best_model_c_v4.pt

Saving criterion: best combined score = SevR2 - PosMAE
(AUROC is 1.0 from epoch 1 on municipal simulation data,
so it cannot be used as checkpoint selection criterion)
...
def compute_save_score(sev_r2: float, pos_mae: float) -> float:
    """
    Combined score for checkpoint selection.
    AUROC is 1.0 from epoch 1 on municipal simulation data so cannot
    be used for checkpoint selection — model would never update.
    Instead we maximise SevR2 (higher is better) and minimise PosMAE
    (lower is better), combined as SevR2 - PosMAE.
    """
...
def train():
    ...
    for epoch in range(start_epoch, CFG["n_epochs"] + 1):
        ...
        if score > best_score:
            ...
            print(f"  Saved best Model C "
                  f"(SevR2={vl['sev_r2']:.4f} PosMAE={vl['pos_mae']:.4f} Score={score:.4f})")
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / "training_curves_c_v4.png", dpi=150)

    with open(MODELS_DIR / "history_c_v4.json", "w") as f:
        json.dump(history, f, indent=2)

    print(f"\nBest Model C score: {best_score:.4f}")
