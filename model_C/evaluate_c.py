"""
evaluate_c.py — Test Evaluation for Model C
=============================================
Reads from ../cache_c/
Loads best_model_c_v3.pt
Saves test_results_c.json
"""
...
def evaluate_split(model, split: str, device) -> dict:
    ...
    model.eval()
    pbar = tqdm(loader, desc=f"Evaluating {split}", unit="batch", dynamic_ncols=True)
...
def plot_results(all_results: list):
    ...
    plt.suptitle("Model C — Test Evaluation (Networks 3, 6, 8)",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / "test_evaluation_c.png", dpi=150)
    print(f"\nSaved: {PLOTS_DIR}/test_evaluation_c.png")


def print_summary(all_results: list):
    print(f"\n{'='*55}")
    print(f"  MODEL C SUMMARY — Cross-Network Generalisation")
    print(f"{'='*55}")
    print(f"  {'Network':<30} {'AUROC':>8} {'F1':>8} {'Acc':>8} {'PosMAE':>8} {'SevR2':>8}")
    print(f"  {'-'*70}")
    for r in all_results:
        pos = f"{r['pos_mae']:.4f}" if r["pos_mae"] else "   N/A"
        sev = f"{r['sev_r2']:.4f}"  if r["sev_r2"] else "   N/A"
        print(f"  {r['network']:<30} {r['auroc']:>8.4f} {r['f1']:>8.4f} "
              f"{r['accuracy']:>8.4f} {pos:>8} {sev:>8}")

    aurocs = [r["auroc"] for r in all_results]
    print(f"\n  Mean AUROC: {np.mean(aurocs):.4f} | Min: {min(aurocs):.4f}")
...
def main():
    ...
    print(f"Changes: {ckpt.get('changes', 'N/A')}")

    all_results = []
    for split in TEST_SPLITS:
        cache_path = CACHE_ROOT / split / "signals.npy"
        if not cache_path.exists():
            print(f"Cache not found for {split} — run pregenerate_c.py first")
            continue
